...
class MCQuestion(BaseModel):
    ...
    explanation_of_correct_answer: str
...
def generate_mc_questions(text, n=5):
    system = f"""Given the corpus of text, \
...
def get_writing_score(writing_text, task_question, test_choice="ielts"):
  ...
  """
    ...
    response = client.beta.chat.completions.parse(
        ...
        response_format=(WritingScoreIELTS if test_choice ==
                         "ielts" else WritingScoreCELPIP),
        temperature=0
    )
    return response.choices[0].message.parsed
